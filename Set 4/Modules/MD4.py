...
class MD4:
	...
	def jumpStart(self, state, stream):
		self.A, self.B, self.C, self.D = state
		msg_bytes = TXT_TO_BYTES(_pad(stream))
		self._compress(msg_bytes[-64:])

	def hexdigest(self):
		s = hex(self.A)[2:].rstrip("L").zfill(8)
		s += hex(self.B)[2:].rstrip("L").zfill(8)
		s += hex(self.C)[2:].rstrip("L").zfill(8)
		s += hex(self.D)[2:].rstrip("L").zfill(8)
