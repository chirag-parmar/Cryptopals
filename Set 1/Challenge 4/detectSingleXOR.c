...
int main(int argc, char *argv[]){
	...
	int lineNum = 0, score, maxScore = 0, found;
	FILE *fptr;
	char str[61], *line, *cracked, temp[60];
	fptr = fopen(argv[1],"r");
	...
	while (fgets(str, 61, fptr) != NULL){		
		if(!strcmp(str, "\n")) continue;
		line = breakSingleXOR(str);
		score = scoreText(line);
		if(score > maxScore){
			maxScore = score;
			found = lineNum;
			cracked = line;
	...
	printf("Line: %d\nMessage: %sScore: %d", found, cracked, maxScore);
